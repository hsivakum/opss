...
const ScmFormApp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const requestData = {
      ...
      type: scmType,
      isPrivate,
      encryptedToken: isPrivate ? scmToken : '',
    };

    // Make API call using Axios
    setLoading(true);
    try {
      const response = await axios.post('http://localhost:8080/api/v1/scan', requestData);
  ...
  return (
      <Container style={{width: 500, height: 750, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
